...
public class CitizensPersistenceManager {
    ...
    public void load() {
        ...
        try (DataInputStream in = new DataInputStream(new BufferedInputStream(new FileInputStream(ASSIGNMENTS_FILE)))) {
            ...
            Freesia.LOGGER.info("[Citizens] Loaded assignments for {} servers", byServerId.size());
        } catch (Exception e) {
            Freesia.LOGGER.warn("[Citizens] Failed to load citizens_assignments.dat: {}", e.getMessage());
        }
    }
    ...
    public Map<String, byte[]> loadModelBinaryCache() {
        ...
        try (DataInputStream in = new DataInputStream(new BufferedInputStream(new FileInputStream(MODEL_CACHE_FILE)))) {
            ...
            Freesia.LOGGER.info("[Citizens] Loaded {} model binary entries", cache.size());
        } catch (Exception e) {
    ...
    private void flush() {
        try (DataOutputStream out = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(ASSIGNMENTS_FILE)))) {
            ...
            Freesia.LOGGER.warn("[Citizens] Failed to save citizens_assignments.dat: {}", e.getMessage());
        }
    }
}
